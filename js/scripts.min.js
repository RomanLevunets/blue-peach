...
//SCROLLIFY
		if ($(window).width() > 992) {
			$.scrollify({
				...
				before: function (index, section) {
					...
					$('#current_number').text(`0${index + 1}`);
				},
				after: function (index, section) {
					$('.nav_links').removeClass('line_add')
					$($('.nav_links')[index]).addClass('line_add');
					// section.forEach((elem, index1) => {
					// 	if($.scrollify.current() === index1){
				...
				afterRender: function () {
					$.scrollify.current().addClass('active')
					$('#current_number').text(`0${$.scrollify.currentIndex() + 1}`);
					$($('.nav_links')[$.scrollify.currentIndex()]).addClass('line_add');
				}
			});
			//SCROLLIFY END
		} else {
			...
			arr.push(...sections);
			arr.push(...footer);
			$('#mob_menu li:not(":last-child") a').click(e => {
				const $th = $(e.delegateTarget);
				const id = $th.parent().index();
		...
		$('.nav_links, #arrow_down a').click(e => {
			e.preventDefault();
			const $th = $(e.delegateTarget);
			const currentSection = $th.attr('href').slice(1);
			$.scrollify.move(parseInt(currentSection))
		})
